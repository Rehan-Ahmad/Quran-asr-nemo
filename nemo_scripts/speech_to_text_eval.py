...
def main(cfg: EvaluationConfig):
    ...
    # Avoid double-percent-formatting when metrics are percent-like (historical inconsistency)
    try:
        # If wer is > 1, assume it's already percentage (e.g. 92.81) and print as: 92.81%
        if isinstance(wer, (int, float)) and wer > 1:
            logging.info(f"Dataset WER/CER {wer:.2f}%/{cer:.2f}%")
        else:
            logging.info(f"Dataset WER/CER {wer:.2%}/{cer:.2%}")
    except Exception:
        logging.info(f"Dataset WER/CER {wer}/{cer}")

    # Log the text_processing config used for the evaluation (helps reproduce training parity)
    try:
        logging.info(f"Text processing used: punctuation_marks={cfg.text_processing.punctuation_marks}, separate_punctuation={cfg.text_processing.separate_punctuation}, do_lowercase={cfg.text_processing.do_lowercase}, rm_punctuation={cfg.text_processing.rm_punctuation}")
    except Exception:
        pass
