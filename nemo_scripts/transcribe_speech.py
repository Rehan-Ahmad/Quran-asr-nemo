...
def main(cfg: TranscriptionConfig) -> Union[TranscriptionConfig, List[Hypothesis]]:
    ...
    # Allow forcing a tokenizer at runtime via env vars to ensure eval/training parity
    tokenizer_dir = os.environ.get("TOKENIZER_DIR", None)
    tokenizer_type = os.environ.get("TOKENIZER_TYPE", "bpe")
    if tokenizer_dir:
        try:
            logging.info(f"Applying runtime tokenizer override from TOKENIZER_DIR={tokenizer_dir}")
            asr_model.change_vocabulary(new_tokenizer_dir=tokenizer_dir, new_tokenizer_type=tokenizer_type)
            # Ensure model modules are on the expected device after changing vocab
            try:
                asr_model.to(map_location)
                logging.info(f"Moved model to device: {map_location} after tokenizer override")
            except Exception:
                logging.warning("Could not move model to map_location after tokenizer override")
            logging.info("Runtime tokenizer override applied successfully")
        except Exception as e:
            logging.warning(f"Failed to apply runtime tokenizer override: {e}")

    # Allow forcing attention/context window size for streaming models via ATT_CONTEXT env var
    # Format: 'L,R' or a single integer 'L' which will be used as [L, 0]
    att_context_env = os.environ.get("ATT_CONTEXT", None)
    if att_context_env is not None:
        try:
            parts = [int(x.strip()) for x in att_context_env.split(",") if x.strip() != ""]
            if len(parts) == 1:
                parts = [parts[0], 0]
            elif len(parts) >= 2:
                parts = parts[:2]
            if hasattr(asr_model, "encoder") and hasattr(asr_model.encoder, "set_default_att_context_size"):
                logging.info(f"Applying ATT_CONTEXT override: {parts}")
                asr_model.encoder.set_default_att_context_size(parts)
                # ensure model is on device after change
                try:
                    asr_model.to(map_location)
                except Exception:
                    pass
        except Exception as e:
            logging.warning(f"Failed to parse/apply ATT_CONTEXT={att_context_env}: {e}")
